...
``interference,'' as the term is used in causal inference), the hypothetical randomization we
...
Treating the instructors as units versus treating the courses as units are conceptually 
different experimental designs, but they lead to the same invariance under the
null hypothesis, and hence the same permutation test.
...
Students of more effective instructors should get higher grades on the final exams, on average,
...
To our knowledge, only two experiments have been able to control for teaching style: \citet{Arbuckle2003} and \citet{MacNell2014}. 
Both experiments suggest that SET reflect gender bias rather than a pedagogical ``match''
of teaching style with student learning style. 
...
``slides of an age- and gender-neutral stick figure and listened to a neutral voice presenting a lecture and then evaluated it on teacher evaluation forms that indicated 1 of 4 different age and gender conditions (male, female, ``old,'' and ``young'')'' [\citealp{Arbuckle2003}, p.507]. 
The goal of the experiment was to measure whether 
``students' perceptions of a professor's age and gender influence their perceptions of the 
professor's warmth and enthusiasm.'' 
Differences in SET could thus be caused only by students' subjective age and gender-biased judgments in evaluating the instructor's competence. 
The researchers find that students rated apparently young male instructors higher than the other three combinations, especially on ``enthusiasm,'' ``showed interest in subject,''  and 
``using a meaningful voice tone.'' 
...
discussion groups for a course, each taught by one of two TAs, one male and one female. 
The two instructors worked together with the main professor of the course, to make sure that they gave similar types of feedback to students, returned assignments at exactly the same time, etc., 
minimizing differences in teaching styles and grading.

Biases in student ratings are evident in the differences in ratings for a given TA when that TA is identified as male versus as female.
...
apparent gender (there are four treatments).
...
Different students might give different scores, and the same student might give the two
TAs different scores.
Because of how the randomization was performed, all allocations of students to TA sections that preserve the number of students in each section are equally likely.  
In particular, all allocations that keep the same students assigned to each actual TA the same are 
equally likely. 

To test whether there is a systematic difference in how students rate apparently male and apparently female TAs, we use the difference in pooled means as our test statistic (Table~\ref{tab:macnell1}).  
That is, we take the mean SET for both instructors when they are identified as female,
minus the mean SET for both instructors when they are identified as male.
The randomization holds constant the set of students assigned to each of the two TAs, since,
under the null hypothesis, we then know what SET students would have given for each possible
allocation.
The randomization includes the nonresponders, who are omitted from the averages of whichever
group they happen to land in.

We also perform tests relating to the association of concordance of student and apparent TA
gender,
(Table~\ref{tab:macnell2}), SET and actual concordance of student and actual TA gender 
(Table~\ref{tab:macnell3}), and grades and actual TA gender(Table~\ref{tab:macnell4}).  
For these tests, we use the Spearman correlation as the test statistic, but find the $p$-value
based on the stratified permutation distribution.
...
\citet{Boring2015} and \citet{MacNell2014}, both of which found that gender biases prevent 
SET from objectively measuring teaching effectiveness. 
Our results confirm that an instructor's perceived gender has a stronger influence on SET
than student-related outcomes such as learning, as measured by performance on the final exam. 
Instructors appear to be rated on a characteristic that is neither relevant nor in their control---their
gender--rather than on their ability to foster student learning. 
We also find that the extent and direction of gender biases appear to depend on context, making it
impossible to adjust for such biases to ``level the playing field.'' 
While the French university setting highlights a positive male student bias for male instructors, 
the experimental US setting suggests a negative female student bias against female instructors.
