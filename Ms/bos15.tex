...
We consider the two primary conclusions to be that 
(1)~SET differ by perceived instructor gender (women are rated lower), and
(2)~SET differ by concordance of student gender and perceived instructor gender (female students
rate male instructors higher).
To account for multiplicity, we tested these two ``omnibus'' hypotheses 
...
In $10^5$ replications, the empirical $p$-values for both omnibus hypotheses were \todo{???},
giving an upper 99\% confidence bound of \todo{????} for the actual $p$-values (the upper bound
was obtained by inverting Binomial hypothesis tests).
